...
router.get('/like/four/images', (req, res) => {
  ...
  connection.query('SELECT image.img_path FROM posts JOIN `Like` ON posts.id = `Like`.PostId JOIN image ON posts.img = image.img_path WHERE `Like`.UserId = ? ORDER BY posts.id DESC LIMIT 4', [userId], (error, results) => {
    if (error) {
      console.error('Error fetching images: ' + error.stack);
      res.status(500).json({ error: 'Internal Server Error' });
      return;
...
router.get('/like/images', (req, res) => {
  ...
  connection.query('SELECT image.img_path FROM posts JOIN `Like` ON posts.id = `Like`.PostId JOIN image ON posts.img = image.img_path WHERE `Like`.UserId = ? ORDER BY posts.id DESC', [userId], (error, results) => {
    if (error) {
      console.error('Error fetching images: ' + error.stack);
      res.status(500).json({ error: 'Internal Server Error' });
      return;
